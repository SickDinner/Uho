...
export class GeniusLifeApp {
  ...
  private setupControls(): void {
    this.controlsPanel.innerHTML = `
      ...
      <div class="controls-hint">Pikanäppäimet: [Space] pause, [F] speed, [D] overlay, [R] reset</div>
    `;
    ...
    modeCalmBtn.addEventListener('click', () => {
      this.applyMode('Calm', intensityInput);
      this.updateModeButtons(modeCalmBtn, modeBalancedBtn, modeChaosBtn);
    });
    modeBalancedBtn.addEventListener('click', () => {
      this.applyMode('Balanced', intensityInput);
      this.updateModeButtons(modeCalmBtn, modeBalancedBtn, modeChaosBtn);
    });
    modeChaosBtn.addEventListener('click', () => {
      this.applyMode('Chaos', intensityInput);
      this.updateModeButtons(modeCalmBtn, modeBalancedBtn, modeChaosBtn);
    });
  ...
  private resetWorld(): void {
    ...
    const fxBtn = document.getElementById('fxBtn') as HTMLButtonElement | null;
    const intensityInput = document.getElementById('intensityInput') as HTMLInputElement | null;
    if (fxBtn) fxBtn.textContent = '✨ Efektit: ON';
  ...
  private triggerIdeaBurst(): void {
    const best = [...this.citizens].sort((a, b) => b.creativity - a.creativity).slice(0, 3);
  ...
  private applyMode(mode: SimMode, intensityInput: HTMLInputElement): void {
    ...
    const intensity = mode === 'Calm' ? 0.7 : mode === 'Balanced' ? 1 : 1.8;
